...
class FileTextExtractor:
    ...
    # 2025-07-21: extract_pdf_text 메서드 들여쓰기 수정 - 클래스 메서드로 인식되지 않는 문제 해결
    @staticmethod
    def extract_pdf_text(file) -> str:
        ...
        try:
            ...
            # 2025-07-21: pdfplumber 사용 시도 및 에러 처리 개선
            try:
                import pdfplumber
                ...
                with pdfplumber.open(file) as pdf:
                    for page_num, page in enumerate(pdf.pages):
                        if page_num >= 50:  # 최대 50페이지까지만
                            text.append(f"\n[총 {len(pdf.pages)}페이지 중 처음 50페이지만 추출했습니다]")
                            break
                        
                        # 표 추출
                        tables = page.extract_tables()
                        if tables:
                            text.append(f"--- 페이지 {page_num + 1} (표) ---")
                            for table in tables:
                                # 표를 구조화된 텍스트로 변환
                                for row in table:
                ...
                logger.warning("pdfplumber가 설치되지 않았습니다. PyPDF2로 폴백합니다.")
    ...
    # 2025-07-21: PyPDF2 폴백 메서드 추가 - pdfplumber 실패 시 대체 방법
    @staticmethod
    def extract_pdf_text_pypdf2(file) -> str:
        """PyPDF2를 사용한 PDF 텍스트 추출 (폴백)"""
        try:
            file.seek(0)
            ...
            logger.error(f"[FileTextExtractor] PyPDF2 텍스트 추출 오류: {str(e)}")
            return f"[PyPDF2 텍스트 추출 오류: {str(e)}]"
...
class CourseDesigner:
    def __init__(self, model_type: str = 'flash'):
        ...
        # 2025-07-21: 모델 선택 및 토큰 제한 개선 - Gemini 2.0 API 실제 제한 반영
        try:
            if model_type == 'pro':
                # 2025-07-21: gemini-2.0-pro-exp는 더 이상 사용되지 않음, gemini-2.5-pro로 대체
                self.model = genai.GenerativeModel('gemini-2.5-pro')
                self.max_output_tokens = 65536  # Pro 모델의 실제 제한 (8배 증가)
            else:
                self.model = genai.GenerativeModel('gemini-2.0-flash')
                self.max_output_tokens = 8192   # Flash 모델의 실제 제한
            
            logger.info(f"[CourseDesigner] 모델 초기화 성공 - 최대 출력 토큰: {self.max_output_tokens}")
        except Exception as e:
            logger.error(f"[CourseDesigner] 모델 초기화 실패: {str(e)}")
            # 2025-07-21: 폴백으로 안정적인 모델 사용
            self.model = genai.GenerativeModel('gemini-1.5-flash')
            self.max_output_tokens = 8192
            logger.info("[CourseDesigner] 폴백 모델 사용: gemini-1.5-flash")
    ...
    # 2025-07-21: 프롬프트 생성 메서드 개선 - JSON 예시와 명확한 지시사항 추가
  ...
  "chapters": [
    {
      ...
      "subchapters": [
        ...
        prompt = f"""
{edu_level_kr} {user_prompt} 코스를 다음 형식의 JSON으로 생성하세요.
{file_context}
규칙:
- 대단원 2-3개
- 각 대단원당 소단원 2-3개
...
"""
    ...
    # 2025-07-21: 전체 코스 생성 프롬프트 개선 - 더 명확한 지시사항
    def create_full_course_prompt(self, 
        ...
        json_example = '''{
  "course": {
    "subject_name": "초등 국어 5-2",
    "description": "5학년 2학기 국어 교과 과정",
    "target": "초등학교 5학년",
    "difficulty_level": "표준/중급"
  },
  "chapters": [
    {
      "chapter_title": "1. 마음을 나누며 대화해요",
      "chapter_order": 1,
      "description": "다양한 상황에서 공감하며 바르게 대화하는 방법을 배웁니다",
      "subchapters": [
        {
          "sub_chapter_title": "1-1. 공감하며 대화하기",
          "sub_chapter_order": 1,
          "description": "공감의 중요성과 방법을 배웁니다",
          "chasis": [
            {
              "chasi_title": "1차시: 공감의 의미 알기",
              "chasi_order": 1,
              "description": "공감의 개념과 중요성을 이해합니다",
              "learning_objectives": "공감의 의미를 설명할 수 있다",
              "duration_minutes": 45
            }
          ]
        }
        ...
        prompt = f"""
당신은 교육 과정 설계 전문가입니다. 
{edu_level_kr} {user_prompt} 코스의 전체 구조를 JSON 형식으로 생성해주세요.

아래 참고자료에서 추출한 모든 단원과 차시를 빠짐없이 포함해야 합니다:
...
중요 지시사항:
1. 참고자료에 있는 모든 대단원, 소단원, 차시를 빠짐없이 포함하세요
2. 차시 번호와 제목은 참고자료와 완전히 동일하게 작성하세요
3. 순서는 참고자료의 순서를 정확히 따르세요
4. 누락된 내용이 없는지 반드시 확인하세요
5. 코드블록(```) 없이 순수 JSON만 출력하세요

JSON 형식 예시:
{json_example}

필수 포함 사항:
- course: 과목 정보 (subject_name, description, target, difficulty_level)
- chapters: 대단원 배열
  - chapter_title: 대단원 제목
  - chapter_order: 대단원 순서
  - description: 대단원 설명
  - subchapters: 소단원 배열
    - sub_chapter_title: 소단원 제목
    - sub_chapter_order: 소단원 순서
    - description: 소단원 설명
    - chasis: 차시 배열
      - chasi_title: 차시 제목
      - chasi_order: 차시 순서
      ...
      - duration_minutes: 수업 시간 (기본 45분)

이제 위 형식에 맞춰 참고자료의 모든 내용을 포함한 완전한 JSON을 생성하세요.
"""
    ...
    def generate_course_structure_stream(self, 
        ...
        try:
            ...
            # 2025-07-21: 전체 코스 생성 모드 감지 개선
            is_full_course_mode = settings.get('generate_full_course', False)
            if not is_full_course_mode and extracted_text:
                # 파일이 있고 특정 키워드가 있으면 자동으로 전체 모드 활성화
                full_course_keywords = ['전체', '모든', '완전한', '파일을 바탕으로', 'pdf', 'PDF']
                if any(keyword in user_prompt for keyword in full_course_keywords):
                    is_full_course_mode = True
                    logger.info("[generate_stream] 전체 코스 생성 모드 자동 활성화")
            ...
            logger.info(f"[generate_stream] 프롬프트 길이: {len(prompt)}, 최대 토큰: {self.max_output_tokens}")
            logger.info("[generate_stream] Gemini API 호출 시작")
            
            # 2025-07-21: 스트리밍 응답 생성 - 실제 토큰 제한 사용
            response = self.model.generate_content(
                prompt,
                stream=True,
                generation_config={
                    "temperature": 0.5 if is_full_course_mode else 0.7,
                    "top_p": 0.9 if is_full_course_mode else 0.95,
                    "max_output_tokens": self.max_output_tokens,  # 모델별 실제 제한 사용
                },
                safety_settings={
            ...
            token_limit_reached = False  # 2025-07-21: 토큰 제한 도달 플래그 추가
            ...
            for chunk in response:
                ...
                try:
                    ...
                    if hasattr(chunk, 'candidates') and chunk.candidates:
                        ...
                        # 2025-07-21: finish_reason 체크 강화
                        if hasattr(candidate, 'finish_reason'):
                            if candidate.finish_reason == 1:  # STOP
                                logger.info(f"[generate_stream] 정상 종료")
                            elif candidate.finish_reason == 2:  # SAFETY
                                logger.warning(f"[generate_stream] 안전성 필터")
                                continue
                            elif candidate.finish_reason in [3, 4, 5]:  # MAX_TOKENS/LENGTH
                                logger.warning(f"[generate_stream] 토큰 제한 도달 - finish_reason: {candidate.finish_reason}")
                                token_limit_reached = True
                                # 토큰 제한에 도달해도 현재까지의 결과로 계속 진행
                    ...
                    if chunk_text:
                        ...
                        if chunk_count - last_progress_update >= 5:
                            ...
                            if '"chapters"' in full_text:
                                ...
                                progress_msg = f"생성 중... ({chapter_count}개 대단원, {subchapter_count}개 소단원, {chasi_count}개 차시)"
                                if token_limit_reached:
                                    progress_msg += " [토큰 제한 도달]"
            ...
            if course_structure:
                ...
                # 2025-07-21: 토큰 제한 경고 추가
                if token_limit_reached:
                    stats['warning'] = "토큰 제한으로 일부 내용이 생략되었을 수 있습니다."
                ...
                yield {
                    ...
                    "data": {
                        ...
                        "token_limit_reached": token_limit_reached  # 2025-07-21: 토큰 제한 정보 추가
                    },
                    "timestamp": datetime.now().isoformat()
                }
                ...
                yield {
                    ...
                    "data": {
                        ...
                        "model_used": self.model_type,
                        "warning": "응답 파싱에 실패하여 부분적인 구조만 생성되었습니다. 다시 시도해보세요.",
                        "mode": "partial"
                    },
                    "timestamp": datetime.now().isoformat()
                }
    ...
    def calculate_statistics_v2(self, structure: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(f"[calculate_statistics] 상세 - 대단원: {stats['total_chapters']}, 소단원: {stats['total_subchapters']}, 차시: {stats['total_chasis']}")
        
        return stats

    def calculate_statistics_v2_07220207(self, structure: Dict[str, Any]) -> Dict[str, Any]:
        """모델 구조에 맞는 통계 계산"""
        logger.info("[calculate_statistics] 통계 계산 시작")
...
def design_course_structure_stream(request):
    ...
    try:
        ...
        settings = {
            ...
            # 2025-07-21: 전체 코스 생성 옵션 추가
            'generate_full_course': request.POST.get('generate_full_course', 'false').lower() == 'true'
        }
    ...
    try:
        ...
        response = designer.model.generate_content(
            ...
            generation_config={
                "temperature": 0.7,
                "top_p": 0.95,
                "max_output_tokens": designer.max_output_tokens,  # 2025-07-21: 모델별 실제 제한 사용
            },
            safety_settings={
                "HARM_CATEGORY_HARASSMENT": "BLOCK_NONE",
                "HARM_CATEGORY_HATE_SPEECH": "BLOCK_NONE",
